...
Let g' a e := fun x => if (x \in (ball a e : set R^o)) then
  (Num.sqrt (s ^+ 2 * pi *+ 2))^-1 *
          expR (- 0%R ^+ 2 / (s ^+ 2 *+ 2))
else
  (Num.sqrt (s ^+ 2 * pi *+ 2))^-1 *
...
Lemma continuous_g' a e : (0 <= e) -> continuous (g' a e).
Proof.
move=> e0.
apply: (@in1TT R).
...
Proof.
move=> e0.
have ballFE x : x <= (a - e)%R -> (x \in (ball a e : set R^o)) = false.
  move=> xae.
  apply: memNset.
...
Proof.
move=> e0.
have ballFE x : a + e <= x -> (x \in (ball a e : set R^o)) = false.
  move=> xae.
  apply: memNset.
