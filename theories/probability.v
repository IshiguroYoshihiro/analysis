...
(*    exponential_pdf m == exponential pdf with mean m when 0 < m             *)
(*   exponential_prob m == exponential probability measure                    *)
(* ```                                                                        *)
(*                                                                            *)
(******************************************************************************)
...
Lemma measurable_normal_fun m s : measurable_fun setT (normal_fun m s).
Proof.
...
Definition normal_pdf (m s x : R) : R :=
  if s == 0 then \1_`[0, 1] x else
  normal_pdf0 m s x.

Lemma normal_pdfE m s : s != 0 -> normal_pdf m s = normal_pdf0 m s.
Proof.
by rewrite /normal_pdf; have [_|s0] := eqVneq s 0.
Qed.
...
(* NB: normal_pdf is not continuous for s = 0 *)
...
rewrite integralZl//=; last exact: integrable_normal_pdf_part.
rewrite integral_normal_pdf_part// -EFinM mulVf// sqrtr_eq0 -ltNge.
by rewrite mulrn_wgt0// mulr_gt0 ?pi_gt0// exprn_even_gt0.
Qed.

Lemma integrable_normal_pdf : mu.-integrable setT
  (fun x => (normal_pdf m sigma x)%:E).
Proof.
apply/integrableP; split.
  apply/measurable_EFinP; apply: measurable_normal_pdf.
...
Lemma derive1_onem {R : realType} : (fun x0 : R => (1 - x0)%R : R^o)^`()%classic = (cst (-1)%R).
Proof.
apply/funext => x.
...
Lemma derive_onemXn {R : realType} (n : nat) x :
  ((fun y : R => `1-y ^+ n.+1 : R^o)^`()%classic x = - n.+1%:R * `1-x ^+ n)%R.
Proof.
...
Lemma XMonemX_le1 a b x : x \in `[0%R, 1%R] -> XMonemX a b x <= 1 :> R.
Proof.
...
Lemma integrable_XMonemX a b :
  mu.-integrable `[0%R, 1%R] (fun x : R => (XMonemX a b x)%:E).
Proof.
...
Proof.
by rewrite /XMonemX01 patchE; case: ifPn => //; rewrite inE/= => /XMonemX_ge0; exact.
Qed.

Lemma XMonemX01_le1 t : XMonemX01 t <= 1.
Proof.
by rewrite /XMonemX01 patchE ; case: ifPn => //; rewrite inE/= => /XMonemX_le1; exact.
Qed.

Lemma measurable_XMonemX01 A : measurable_fun A XMonemX01.
Proof.
apply: measurable_funTS; apply/(measurable_restrictT _ _).1 => //.
by apply/measurable_funTS; exact: measurable_fun_XMonemX.
Qed.
...
Lemma beta_fun00 : beta_fun 0 0 = 1%R :> R.
Proof.
rewrite -[LHS]Rintegral_mkcond.
under eq_Rintegral do rewrite XMonemX00.
rewrite Rintegral_cst//= mul1r.
by rewrite lebesgue_measure_itv/= lte_fin ltr01 -EFinB subr0.
Qed.

Lemma beta_fun_sym a b : beta_fun a b = beta_fun b a :> R.
Proof.
rewrite -[LHS]Rintegral_mkcond Ronem_change//=; last 2 first.
...
Lemma beta_funS a b :
  beta_fun a.+2 b.+1 = a.+1%:R / b.+1%:R * beta_fun a.+1 b.+2 :> R.
Proof.
rewrite -[LHS]Rintegral_mkcond.
...
Lemma beta_funSS a b : beta_fun a.+1 b.+1 =
  a`!%:R / (\prod_(b.+1 <= i < (a + b).+1) i)%:R * beta_fun 1 (a + b).+1 :> R.
Proof.
elim: a b => [b|a ih b].
...
Lemma beta_fun1S b : beta_fun 1 b.+1 = b.+1%:R^-1 :> R.
Proof.
...
Lemma beta_fun_fact a b :
  beta_fun a.+1 b.+1 = (a`! * b`!)%:R / (a + b).+1`!%:R :> R.
Proof.
...
Lemma beta_fun_ge0 a b : 0 <= beta_fun a b :> R.
...
Proof.
have := beta_fun_gt0 a b; rewrite /beta_fun /Rintegral /beta_funEFin.
by case: (integral _ _ _) => [r _| |//]; rewrite ?ltxx ?ltry.
Qed.

Let beta_funEFin_fin_num a b : beta_funEFin a b \is a fin_num.
Proof.
rewrite ge0_fin_numE ?beta_funEFin_lty//.
...
Proof.
by rewrite /beta_pdf divr_ge0//; [exact: XMonemX01_ge0|exact: beta_fun_ge0].
Qed.
...
rewrite -beta_funEFinT -ge0_integralZr//=.
...
by rewrite mulVf ?XMonemX01_le1// gt_eqF// beta_fun_gt0.
...
Proof.
rewrite /beta_prob /= /mscale /= beta_numT beta_funEFinE//.
...
Proof.
rewrite /beta_prob /= /mscale/= /beta_num -beta_funEFinT beta_funEFinE//.
by rewrite -EFinM mulVf// gt_eqF// beta_fun_gt0.
Qed.
...
rewrite /beta_prob/= /mscale/= /beta_num lte_mul_pinfty//.
  by rewrite lee_fin// invr_ge0 beta_fun_ge0.
apply: (@le_lt_trans _ _ (beta_funEFin a b)).
  apply: ge0_subset_integral => //=.
    by apply/measurable_EFinP; exact: measurable_XMonemX01.
  by move=> x _; rewrite lee_fin XMonemX01_ge0.
by rewrite beta_funEFin_lty.
Qed.
...
rewrite integral_XMonemX01; apply/eqP; rewrite eq_le; apply/andP; split; last first.
  by apply: integral_ge0 => x _; rewrite lee_fin XMonemX01_ge0.
apply: (@le_trans _ _ (\int[mu]_(x in A `&` `[0%R, 1%R]%classic) 1)%E); last first.
...
apply: (@integrableB _ (g_sigma_algebraType R.-ocitv.-measurable)) => //=.
  ...
  under eq_integral.
    ...
    over.
  by rewrite /= -/(beta_funEFin a b) /= beta_funEFinT.
...
Lemma integral_beta_prob a b f U : measurable U -> measurable_fun U f ->
  ...
  (\int[beta_prob a b]_(x in U) f x = \int[mu]_(x in U) (f x * (beta_pdf a b x)%:E) :> \bar R)%E.
Proof.
move=> mU mf finf.
...
under eq_integral => x.
  ...
  rewrite bernoulli_probE/= ?XMonemX01_ge0 ?XMonemX01_le1//.
  over.
rewrite /=.
...
rewrite integralB_EFin//=; last 2 first.
  exact: integrableS (integrable_XMonemX01 _ _).
  exact: integrableS (integrable_XMonemX01 _ _).
rewrite -!beta_funEFinT !beta_funEFinE.
rewrite -EFinM.
rewrite mulrBl /onem mulfV; last by rewrite gt_eqF// beta_fun_gt0.
by [].
Qed.
...
Definition exponential_prob {R : realType} (m : R) :=
...
Lemma exponential_prob_itv_0bnd (x : R) : 0 < x ->
  exponential_prob mean `[0, x] = (1 - (expR (- x / mean))%:E)%E.
Proof.
move=> x0.
...
Local Notation exponential := (exponential_prob mean).
...
Lemma gt0_cvgMlNy_new {R : realFieldType} (F : set_system R)
  (M : R) (f : R -> R) :
   Filter F ->
    (0 < M)%R ->
      (f r) @[r --> F] --> -oo -> (f r * M)%R @[r --> F] --> -oo.
Proof.
move=> filtF M0 /cvgrNyPle fy; apply/cvgrNyPle => A.
by apply: filterS (fy (A / M)) => x; rewrite ler_pdivlMr.
Qed.

Lemma gt0_cvgMly_new {R : realFieldType} (F : set_system R) (M : R)
 (f : R -> R) :
  Filter F -> (0 < M)%R ->
  f r @[r --> F] --> +oo -> (f r * M)%R @[r --> F] --> +oo.
Proof.
move=> filtF M0 /cvgryPge fy; apply/cvgryPge => A.
apply: filterS (fy (A / M)) => x.
by rewrite ler_pdivrMr.
Qed.

(* checking generalized *)
Example gt0_cvgMlNy' {R : realFieldType}
  (M : R) (f : R -> R) :
    (0 < M)%R ->
      (f r) @[r --> -oo] --> -oo -> (f r * M)%R @[r --> -oo] --> -oo.
Proof.
exact: gt0_cvgMlNy_new.
Abort.

Example gt0_cvgMly' {R : realFieldType} (M : R) (f : R -> R) : (0 < M)%R ->
  f r @[r --> +oo] --> +oo -> (f r * M)%R @[r --> +oo] --> +oo.
Proof.
exact: gt0_cvgMly_new.
Abort.
...
Local Notation ln := (@ln R).

Lemma lnNy : ln x @[x --> (@GRing.zero R)^'+] --> -oo.
Proof.
apply/cvgrNyPle.
move=> y; near=> x.
rewrite -ler_expR lnK; last by rewrite posrE.
near: x.
apply: nbhs_right_le.
exact: expR_gt0.
Unshelve. end_near. Qed.
...
Proof.
move=> x0.
apply: (@cvg_trans _ ((expR (x * ln a)) @[a --> 0^'+])).
  apply: near_eq_cvg.
  near=> a.
  by rewrite /powR gt_eqF.
apply: (@cvg_comp _ _ _ _ _ _ (@ninfty_nbhs R)).
  under eq_cvg do rewrite mulrC.
    apply: gt0_cvgMlNy_new; first exact: x0.
    exact: lnNy.
...
Proof.
move=> v0 a.
rewrite in_itv/= andbT => a0.
apply: (@near_eq_derivable _ _ _
    (fun a' : R^o => expR (x * ln a') : R^o)) => //.
  near=> b.
  rewrite /powR gt_eqF//.
  near: b.
  exact: lt_nbhsr.
apply: diff_derivable.
apply: (@differentiable_comp _ _ R^o); last first.
...
move=> x; rewrite in_itv/= andbT => x0.
rewrite derive1E.
rewrite (@near_eq_derive _ R^o R^o _ (fun x => expR (a * ln x)))//; last first.
  near=> z; rewrite /powR gt_eqF//; near: z.
  exact: lt_nbhsr.
rewrite -derive1E.
...
rewrite mulrC.
rewrite powRB; last by rewrite (@gt_eqF _ _ x)//; apply: implybT.
rewrite powRr1 ?ltW//.
...
transitivity (@exponential_prob R 1 `[0, +oo[).
  ...
  rewrite subrr powRr0.
  by rewrite exponential_pdfE// /exponential_pdf' invr1 mulr1.
